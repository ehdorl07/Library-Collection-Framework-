 ...
 * 
 * -배열 문제점
 * 		:1)저장할수있는 객체수가 배열을 생성할때 결정됨.
 * 			불특정 다수의 개체를 저장하기에는 문제가 됨.
 * 		 2)객체삭제했을 때 해당 인덱스가 비우게됨.
 * 			객체저장하려면 어디가 비어있는지 확인해야함.
 * 
 * -컬렉션프레임워크
 * :객체들을 효율적으로 추가,삭제,검색할수있도록 제공되는  컬렉션 라이브러리.
 * 	인터페이스를 통해서 정형화된 방법으로 다양한 컬렉션 클래스 이용.
 * -컬렉션은 제네릭(Generic)기법으로 구현됨.
 * -제넥릭:특정타입만 다루지 않고, 여러종류의 타입으로 변신할 수 있도록 클래스나 메소드를 일반화시키는 기법.
 * - <E>,<K>,<V>: 타입 매개변수(요소타입을 일반화한 타입)
 * -예) 제넥릭 벡터 : Vector<E>
 *        E에 특정타입으로 구체화,
